...
var initTaskCompletion = function() {
    ...
    $(this).click(function(e) {
      ...
      // if we`re one page that contains project progress bars
      if ($(".project-progress").exists()) {
        refreshProjectDoneness($(this).data('project-id'));
      };
    });
  });
};
...
var initTaskModal = function() {
  $('#task-modal').on('shown', function() {
    $(this).find('[name="task[description]"]').focus();
  });

  $(document).on('click', 'a.task-button', function(event) {
    event.preventDefault();
...
var refreshProjectDoneness = function(project) {
    $.ajax({
      url: '/ajax/project-doneness/' + project,
      type: 'GET',
      success: function(data) {
        // update the progress bar
        $(".project-progress#" + project).width(data + '%')
        $(".progress-number#" + project).html(data + '%')
      },
      error: function(data) {
        set_message('Error refreshing doneness', 'error');
      }
    });
  };
